...
- 修复了“带持续经营重大不确定性段落的无保留意见”被误判为“标准无保留意见”的问题，并将审计意见原文纳入带页码的证据链。
- 为营业收入、净利润、经营现金流、应收账款、存货、资产/负债等主表关键字段补齐了页码证据锚点，风险报告开始优先引用主表原文而不只依赖附注。
- 让坏账准备、存货跌价准备等已提取附注数据进入保守型检测规则，并在证据链不完整时将自动投资建议降级为“待核验”。
- 收口了“待核验”场景下的建议区口径，避免报告同时出现“暂不采用自动投资建议”和自动生成的高/低风险摘要建议。
- 已将会计政策/估计变更、历史违规、审计师变更、或有事项、表外融资、研发资本化、勾稽异常、现金存量/利息错配等治理类/附注类信号接入 `note:*` 页码证据链，减少抽象证据输出。
...
- ~~修复持续经营重大不确定性审计意见的识别误判，并将审计意见原文接入风险证据链。~~
- ~~为主表关键字段补齐页码证据锚点，让主表型风险信号也能直接引用原文。~~
- ~~在证据链不完整时将自动投资建议降级为“待核验”，避免直接给出低风险增持结论。~~
...
- 将更多治理类/附注类信号补成“原文页码 + 摘录 + 计算依据”的完整证据对象，减少剩余的证据断链。
